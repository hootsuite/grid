...
Grid.prototype.generatePositionsFromIndex = function() {
  ...
  for (var i = 0; i < this.items.length; i++) {
    this.generateItemPositionFromIndex(this.items[i]);
  }
  return this.items;
};

Grid.prototype.generateItemPositionFromIndex = function(item) {
  ...
  if (itemPosition) {
    ...
    this.startNewGroup();
    this.generateItemPositionFromIndex(item);
  }
};
...
GridGroup.prototype.itemFitsAtPosition = function(item, position) {
   ...
   */
  var x, y, row;
  // Make sure the item doesn't go outside the bottom of the grid
  if (position[1] + item.rows > this.length) {
    return false;
  }

...
Grid.prototype.generateIndexesFromPosition = function(item) {
...
Grid.prototype._sortItemsByPosition = function() {
  ...
  this.items.sort(function(item1, item2) {
    ...
    // If the items are in the same group and on the same row or column, a user
    // callback will be called to decide the order of these two items (if the
    // callback is passed on when initializing the Grid object.)
    // If the user callback return a neutral value---the zero number, the
    // sorting continues and if there's no row or column different between the
    // two items, the sorting algorithm will do nothing to their indices
    if (item1.position.row == item2.position.row ||
        item1.position.col == item2.position.col) {
      if (typeof(_this.options.sortCollidingItems) == 'function') {
        sortResult = _this.options.sortCollidingItems.apply(this, arguments);
        if (sortResult) {
          return sortResult;
        }
      }
...
Grid.prototype._startNewGroup = function() {
  this.groups.push(new GridGroup(this.options.colsPerGroup, this.options.rows));
};

Grid.prototype._getCurrentGroup = function() {
  return this.groups[this.groups.length - 1];
};

Grid.prototype._getIndexOfItem = function(item) {

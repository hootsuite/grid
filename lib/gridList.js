var GridList = function(items, options) {
  ...
  this.items = items;
  // TODO: Position items if they don't come with 2d positions beforehand
  this.createGrid();
};
...
GridList.prototype.createGrid = function(generatePositions) {
  // We always iterate through items sorted by their grid position
  this._sortItemsByPosition();
  this._resetGrid();
  // We can generate the item positions or simply map existing ones to the grid
  // matrix stricture. Generating is used when changing the grid size

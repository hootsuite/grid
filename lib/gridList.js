...
GridList.prototype = {
  ...
  _itemSpansMoreThanOneSection: function(item) {
  ...
  _findPositionForItem: function(item, start, fixedRow) {
    ...
    for (x = start.x; x < this.grid.length; x++) {
      ...
      if (this.options.columnsPerGroup && this._itemSpansMoreThanOneSection({'x': x, 'w': item.w})) {
        continue;
      }
    ...
    if (this.options.columnsPerGroup) {
      ...
      if (!this._itemSpansMoreThanOneSection({'x': this.grid.length, 'w': item.w})) {
        x = this.grid.length;
        y = fixedRow || 0;
        return [x, y];

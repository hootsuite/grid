...
GridList.prototype.createGrid = function() {
  var currentColumn = 0,
      item,
      i;
...
GridList.prototype.rebuildGrid = function() {
  /**
...
GridList.prototype.moveItemToPosition = function(item, position) {
  ...
  if (!this._resolveCollisionsLocally(item, collidingItems)) {
    this._repositionItemsAroundItem(item);
  }
};
...
GridList.prototype._resolveCollisionsLocally = function(item, collidingItems) {
  /**
   * Attempt to resolve the collisions after moving a an item over one or more
   * other items within the grid, by shifting the position of the colliding
   * items around the moving one. This might result in subsequent collisions,
   * in which case we will revert all position permutations and return false.
   * To be able to revert to the initial item positions, we create a virtual
   * grid in the process
   */
...
GridList.prototype._repositionItemsAroundItem = function(item) {
  var otherItem,
      i,
      position;
  ...
  this._updateItemPosition(item, [item.x, item.y]);
  for (i = 0; i < this.items.length; i++) {
    otherItem = this.items[i];
    if (otherItem == item) {
      continue;
    }
    position = [otherItem.x, otherItem.y];
    // Only reposition item if it no longer fits in its previous position
    if (!this._itemFitsAtPosition(otherItem, position)) {
      position = this._findPositionForItem(
        otherItem, this._findRowTail(otherItem.y, item), otherItem.y);
    }
    this._updateItemPosition(otherItem, position);
  }
};

GridList.prototype._findRowTail = function(row, ignoreItem) {
  // FIXME: understand better
  var i,
      item,
      tail = 0;
  for (i = 0; i < this.grid.length; i++) {
    item = this.grid[i][row];
    if (!item || item == ignoreItem) {
      continue;
    }
    tail = item.x + item.w;
  }

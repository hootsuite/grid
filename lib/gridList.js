...
GridList.prototype._findPositionForItem = function(item, start, fixedRow) {
  /**
   * This method has two options for the position we want for the item.
   * - Starting from a certain row/column number and only looking for positions
   * to its right
   * - Accepting positions for a certain row number only (use-case: items being
   *   shifted to the left/right as a result of collisions)
   */
  var x, y, position;
...
GridList.prototype._ensureColumns = function(N) {
  /**
...
GridList.prototype._deleteItemPositionFromGrid = function(item) {
  var x, y;
  ...
  for (x = item.x; x < item.x + item.w; x++) {
    for (y = item.y; y < item.y + item.h; y++) {
        ...
        // the meantime
        // TODO(andrei): can you give me an example for this?
...
GridList.prototype._resolveCollisions = function(item) {
  collisionsResolved = this._tryToResolveCollisionsLocally(item);
...
GridList.prototype._tryToResolveCollisionsLocally = function(item) {
  ...
  for (i = 0; i < collidingItems.length; i++) {
    ...
    if (_gridList._itemFitsAtPosition(collidingItem, leftOfItem)) {
      ...
      // Collisions failed, we must use the pullToLeft(item) to arrange
